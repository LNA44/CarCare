		...
		try performSync { context in
			Result {
				try ManagedBike.find(in: context).first.map {
					LocalBike(id: $0.id, year: Int($0.year), model: $0.model, brand: Brand(rawValue: $0.brand) ?? .Unknown, mileage: Int($0.mileage), bikeType: BikeType(rawValue: $0.bikeType) ?? .Manual)
				}
			}
		}
	}
