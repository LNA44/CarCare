...
private extension Array where Element == LocalMaintenance {
	func toModels() -> [Maintenance] {
		map { Maintenance(id: $0.id, maintenanceType: MaintenanceType(rawValue: $0.maintenanceType) ?? .Unknown, date: $0.date) }
	}
}

private extension Maintenance {
	func toLocal() -> LocalMaintenance {
		LocalMaintenance(id: id, maintenanceType: maintenanceType.rawValue, date: date)
	}
}
